...
with tab2_Descriptive_Statistics:
	...
	with col_departures:
		...
		).properties(height=700,title=alt.TitleParams(text='Flights per Hour', align='center', subtitle='Departure Flights'),
	...
	with col_arrivals:
		...
		chart = alt.Chart(hourly_flights).mark_bar().encode(
			...
			alt.datum.Count >= hourly_flights.nlargest(2, 'Count')['Count'].min(),
			alt.value('orange'),alt.value('gray'))
		).properties(height=700,title=alt.TitleParams(text='Flights per Hour', align='center', subtitle='Arrival Flights'),
		).configure_axis(grid=False
		).configure_title(fontSize=20,fontWeight='bold')
		st.altair_chart(chart, use_container_width=True)
